...
RedisNamespaceClient.prototype.generate_key = function(key) {
    return this.key_prefix + ":" + key;
}

RedisNamespaceClient.prototype.on = function() {
  return this.redis.on.apply(this.redis,arguments);
}

Object.keys(COMMANDS).forEach(function(method_name) {
  RedisNamespaceClient.prototype[method_name] = function() {
    args = arguments;
    if(COMMANDS[method_name] == FIRST_KEY) {
      args[0] = this.generate_key(args[0]);
    }
    else if(COMMANDS[method_name] == LAST_KEY) {
      args[args.length-1] == this.generate_key(args[args.length-1]);
    }
    ...
    //4: everything is a key
      ...
      for(var i = loop_start; i < loop_end; i++) {
        args[i] = this.generate_key(args[i]);
      }
    }
